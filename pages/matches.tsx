...
export default function SelectMatchDropBox() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            setJsonData(data);
            setMatchNumbers(data.map(
                (item: { match_number: number, comp_level: any, set_number: any}) =>
                    (item.comp_level == "sf" ? item.comp_level + item.set_number : item.comp_level) +
                    item.match_number
            ));
        }
        fetchData();
    ...
    const selectMatch = (match: any) => {
        ...
        jsonData.forEach((item: any) => {
            if (item.comp_level + item.match_number == match) {
                const red = item.alliances.red.team_keys.map(
                    (team) => team.substring(3)
                );
                const blue = item.alliances.blue.team_keys.map(
                    (team) => team.substring(3)
                );
                setBlueTeams(blue);
                setRedTeams(red);
